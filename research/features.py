...
def make_features(
    ...
    if bars_per_hour <= 0:
        raise ValueError("bars_per_hour must be positive")
    short_window = 4 * bars_per_hour
    daily_window = 24 * bars_per_hour
    breakout_window = 48 * bars_per_hour
    ...
    for i, bar in enumerate(bars):
        ...
        if i < breakout_window + 1:
            row["ready"] = 0
            rows.append(row)
            continue
        returns = [safe_return(bars[j - 1].close, bars[j].close) for j in range(i - daily_window + 1, i + 1)]
        ranges = [bars[j].high - bars[j].low for j in range(i - daily_window + 1, i + 1)]
        volumes = [bars[j].volume for j in range(i - daily_window + 1, i + 1)]
        value = sum(bars[j].close * bars[j].volume for j in range(i - daily_window + 1, i + 1))
        volume = sum(volumes)
        prior = bars[i - breakout_window:i]
        high, low = max(b.high for b in prior), min(b.low for b in prior)
        span = high - low
        btc_now, btc_prev = btc_by_ts.get(bar.ts), btc_by_ts.get(bars[i - daily_window].ts)
        btc_ret = safe_return(btc_prev.close, btc_now.close) if btc_now and btc_prev else 0.0
        ret24 = safe_return(bars[i - daily_window].close, bar.close)
        row.update({
            "ready": 1,
            "ret_4": safe_return(bars[i - short_window].close, bar.close),
            "ret_24": ret24,
            "reversal_1": -safe_return(bars[i - bars_per_hour].close, bar.close),
            "rv_24": pstdev(returns) * sqrt(daily_window),
            "atr_24": mean(ranges),
            ...
            "taker_imbalance_24": sum(
                2 * bars[j].taker_buy_volume - bars[j].volume
                for j in range(i - daily_window + 1, i + 1)
            ) / volume if volume else 0.0,
