...
def per_symbol_audit(universe: dict[str, SymbolSeries]) -> dict:
    """Data, funding and input-integrity audit before any performance calculation."""
    rows = {}
    funding_step = 8 * HOUR_MS
    for symbol in BASKET:
        ...
        # Binance funding timestamps can carry millisecond jitter and the funding
        # interval can legitimately shorten (for example from 8h to 4h).  Audit
        # missing coverage as gaps greater than the maximum normal 8h interval,
        # rather than requiring exact membership on an artificial UTC grid.
        missing_funding = []
        for previous, current in zip(item.funding_ts, item.funding_ts[1:]):
            missing_slots = max(0, (current - previous - 60_000) // funding_step)
            missing_funding.extend(previous + slot * funding_step for slot in range(1, missing_slots + 1))
        expected_funding_count = len(item.funding) + len(missing_funding)
        bars_path = DATA_ROOT / "normalized-basket" / f"{symbol}-1h.csv"
        funding_path = DATA_ROOT / "normalized-basket" / f"{symbol}-funding.json"
        rows[symbol] = {
            **audit,
            "funding_events": len(item.funding),
            "expected_funding_events": expected_funding_count,
            "missing_funding_events": len(missing_funding),
            "missing_funding_first_iso": utc_iso(missing_funding[0]) if missing_funding else None,
            "missing_funding_last_iso": utc_iso(missing_funding[-1]) if missing_funding else None,
            "funding_coverage": (
                len(item.funding) / expected_funding_count
                if expected_funding_count
                else 0.0
            ),
            ...
            "input_sha256": {
                "bars_csv": _sha256_file(bars_path),
                "funding_json": _sha256_file(funding_path),
            },
        }
    return rows
