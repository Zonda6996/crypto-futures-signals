...
def calibrate_candidate(candidate: Candidate, features: Sequence[dict], indices: Sequence[int]) -> Calibration:
...
def evaluate_candidate(candidate: Candidate, bars: Sequence[Bar], features: Sequence[dict], indices: Sequence[int], costs: CostModel,
                       funding_by_ts: dict[int, float], calibration: Calibration | None = None) -> tuple[list[Trade], dict]:
    usable = [i for i in indices if features[i].get("ready") and features[i].get(candidate.feature) is not None]
    frozen = calibration or calibrate_candidate(candidate, features, indices)
    threshold = frozen.threshold
    rv_median = frozen.rv_median
...
def search(bars: Sequence[Bar], features: Sequence[dict], funding: Sequence[tuple[int, float]], costs: CostModel, limit: int | None = None) -> dict:
    ...
    for candidate in grid:
        calibration = calibrate_candidate(candidate, features, train_indices)
        _, train_result = evaluate_candidate(candidate, bars, features, train_indices, costs, funding_map, calibration)
        _, validation_result = evaluate_candidate(candidate, bars, features, validation_indices, costs, funding_map, calibration)
        records.append({
...
def robustness(candidate: Candidate, bars: Sequence[Bar], features: Sequence[dict], indices: Sequence[int],
               funding: Sequence[tuple[int, float]], calibration: Calibration) -> dict:
    scenarios = {}
    for name, costs in (
        ("base", CostModel()),
        ("costs_150pct", CostModel(7.5, 1.5, 3.0)),
        ("costs_200pct", CostModel(10, 2, 4)),
    ):
        trades, result = evaluate_candidate(candidate, bars, features, indices, costs, dict(funding), calibration)
