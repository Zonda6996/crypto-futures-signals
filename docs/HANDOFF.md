...
**Новый engine — Phase 1A STOP:** отдельный protocol `ALT-MULTITF-003` заморожен в [`docs/ALTCOIN_MULTITF_FROZEN_PROTOCOL.md`](./ALTCOIN_MULTITF_FROZEN_PROTOCOL.md). Phase 1 разделена на acquisition/sealing (1A) и normalization/eligibility audit (1B). Pre-download gate 1A обнаружил блокер: в versioned inputs нет полного point-in-time Binance USD-M lifecycle registry с delisted/failed contracts; current roster использовать запрещено. Поэтому market-data download, holdout reads, signals/PnL/backtest не выполнялись, holdout `[2026-01-01T00:00:00Z, 2026-08-01T00:00:00Z)` остался закрыт. Отчёт: [`reports/ALTCOIN_MULTITF_PHASE1A_ACQUISITION.md`](../reports/ALTCOIN_MULTITF_PHASE1A_ACQUISITION.md). Handoff следующего чата: [`docs/ALTCOIN_MULTITF_PHASE1B_NEXT_CHAT.md`](./ALTCOIN_MULTITF_PHASE1B_NEXT_CHAT.md). Следующий шаг — владелец предоставляет/утверждает полный registry source; затем повторяется Phase 1A. Phase 1B до PASS 1A запрещена.
