...
**Новый engine — Phase 1A BLOCKED:** отдельный protocol `ALT-MULTITF-003` заморожен в [`docs/ALTCOIN_MULTITF_FROZEN_PROTOCOL.md`](./ALTCOIN_MULTITF_FROZEN_PROTOCOL.md). Повторный pre-download gate проверил официальные источники: Vision listing содержит 986 kline-prefixes (832 `*USDT`, XML SHA-256 `d24a13b22caa8e2251aab3abe76762a76d2e909f636bbf393d4dd9e842dcc38f`), но не authoritative onboard/delist timestamps и не доказательство полноты; `exchangeInfo` является post-holdout current roster и из sandbox отвечает HTTP 451; единого официального machine-readable delisting registry нет. Поэтому biased universe не скачивался: raw market files `0`, holdout reads `0`, signals/PnL/backtest `0`; holdout `[2026-01-01T00:00:00Z, 2026-08-01T00:00:00Z)` не создавался и не открывался. Добавлена fail-closed acquisition/sealing infrastructure и 14 data/sealing tests проходят. Отчёт: [`reports/ALTCOIN_MULTITF_PHASE1A_ACQUISITION.md`](../reports/ALTCOIN_MULTITF_PHASE1A_ACQUISITION.md). Handoff: [`docs/ALTCOIN_MULTITF_PHASE1B_NEXT_CHAT.md`](./ALTCOIN_MULTITF_PHASE1B_NEXT_CHAT.md). Следующий шаг — owner предоставляет/утверждает полный versioned registry source; затем повторяется Phase 1A. Phase 1B до PASS 1A запрещена.
...
Диагностика выполнена отдельно от Phase 2–4 на полном pre-TEST TRAIN+VALIDATION-срезе 2021–2024 при frozen-параметрах и расходах 0,10%. Это описательный анализ конц��нтрации, не новая OOS-оценка. BTC-режимы размечены каузально: trailing 90-day return и trailing 30-day realized volatility; пороги — expanding median только предшествующих валидных наблюдений.
