...
**Новый engine — Phase 1A READY TO RESUME:** protocol `ALT-MULTITF-003` получил owner amendment A1 до скачивания данных и расчётов. Владелец разрешил заморозить текущий Binance USD-M USDT perpetual roster и явно принял survivorship/coverage bias; полный historical registry delisted/failed contracts больше не является gate. Raw market files пока `0`, holdout reads `0`, signals/PnL/backtest `0`; holdout `[2026-01-01T00:00:00Z, 2026-08-01T00:00:00Z)` ещё не создавался и не открывался. Fail-closed acquisition/sealing infrastructure и 14 data/sealing tests готовы. Следующий шаг — отдельный чат выполняет только Phase 1A: сохраняет immutable roster snapshot, скачивает raw `5m`/funding/metadata, физически отделяет holdout и создаёт SHA-256 manifest. Отчёт: [`reports/ALTCOIN_MULTITF_PHASE1A_ACQUISITION.md`](../reports/ALTCOIN_MULTITF_PHASE1A_ACQUISITION.md). Prompt: [`docs/ALTCOIN_MULTITF_PHASE1B_NEXT_CHAT.md`](./ALTCOIN_MULTITF_PHASE1B_NEXT_CHAT.md). Phase 1B до фактического PASS 1A запрещена.
