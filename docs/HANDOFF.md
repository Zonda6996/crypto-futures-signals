...
**Новый engine — Phase 1A BLOCKED на official current roster access:** protocol `ALT-MULTITF-003` сохраняет owner amendment A1 и получил до расчётов amendment A2: cohorts `$10m–25m`/`≥$25m`, age `30d`, period-local gaps, параметры только по TF-группам, hard safety gates отдельно от diagnostic scorecard и development gate по positive net expectancy/приемлемому drawdown/устойчивости большинства окон; одноразовый строгий holdout не ослаблен. На `2026-08-21T22:08:13Z` основной официальный `exchangeInfo` вернул HTTP 451, официальные `fapi1–4` — HTTP 202 с пустым body. Поэтому roster не заморожен, market files `0`, holdout reads `0`, signals/PnL/backtest `0`; historical-delisted blocker не применяется. Нужен egress к непустому официальному production `GET /fapi/v1/exchangeInfo`, после чего Phase 1A начинается заново с immutable roster snapshot. Отчёт: [`reports/ALTCOIN_MULTITF_PHASE1A_ACQUISITION.md`](../reports/ALTCOIN_MULTITF_PHASE1A_ACQUISITION.md). Phase 1B запрещена до фактического PASS 1A.
