...
Лучший найденный исследовательский кандидат — ETHUSDT long при сильном положительном отклонении от 24-часового VWAP, медвежьем режиме BTC и высокой волатильности.

На validation при смоделированных полных расходах 0,10% он показал:

- 43 сделки;
- expectancy +0,286R на сделку;
- итог +12,30R;
- Profit Factor 2,04;
- win rate 58,14%;
- максимальную просадку −3,15R.

Однако на train кандидат был почти нейтральным: PF 1,048, а доверительный интервал expectancy включал ноль. Поэтому это **лучший результат на текущий момент, но не подтверждённый edge и не готовая торговая стратегия**.

Подробности и исходные результаты находятся в:

- `reports/BEST_RESULT_SO_FAR.md`;
- `reports/raw/diagnostic-bingx-costs.json`;
- `reports/scripts/diagnostic_rerun.py`.
...
1. Получить фактическую выписку BingX по одному полному входу и выходу и зафиксировать, является ли 0,05% ставкой за сторону или за круг.
2. Исправить модель расходов под фактическую схему аккаунта.
3. Добавить полный trade log и расчёт R, MAE/MFE, funding и причин выхода.
4. Провести аудит исполнения backtest.
5. Запустить walk-forward текущего замороженного ETH-кандидата.
6. Построить карту устойчивости параметров.
7. Провести bootstrap и анализ концентрации прибыли.
8. Решить, заслуживает ли кандидат открытия закрытого test.
9. Параллельно начать отдельный эксперимент ETH/BTC relative strength.
10. После подтверждения test перейти к paper trading.

## Текущее решение

ETH-кандидат не следует заранее считать ни рабочим, ни гарантированно провальным. Его необходимо проверить без искусственного удушения: основным считать фактический cost BingX, стресс-сценарий использовать только как дополнительный тест, а отдельные убыточные окна анализировать, но не делать автоматическим veto.
