...
Результат переживает разные окна и cost-сценарии, но лучшие пять сделок дают 58–79% total R при 0,10%, а multiple-testing риск не исчез.

Phase 3 проверила заранее заданную сетку из 256 соседних конфигураций на clean VALIDATION с калибровкой только по TRAIN. При 0,10% положительны 90,2% всей сетки, медианный expectancy составляет +0,153R; исходная точка и восемь ближайших соседей положительны все 9 из 9 с медианой +0,246R. Положительный локальный кластер сохраняется при расходах 0,05%, 0,12% и 0,16%. Это снижает риск одиночного параметрического пика, но не устраняет малую выборку, концентрацию прибыли и multiple-testing происхождение гипотезы.

Статус: **кандидат прошёл walk-forward и карту устойчивости параметров, но это ещё не подтверждённый edge и не готовая торговая стратегия**. Закрытый TEST не открыт.

Актуальные подробности:

- `reports/PHASE1_AUDIT.md`;
- `reports/PHASE2_WALK_FORWARD.md`;
- `reports/PHASE3_PARAMETER_STABILITY.md`;
...
Признак edge — положительный кластер параметров. Прибыль в одной узкой комбинации является признаком ��ереобучения.
...
Phase 1 (аудит), Phase 2 (anchored/rolling walk-forward) и Phase 3 (карта устойчивости 256 параметрических точек) завершены. Следующий единственный шаг — Phase 4: заранее описанная проверка экономического механизма текущего сигнала только внутри TRAIN+VALIDATION. Торговые параметры не менять, лучший объясняющий срез не превращать в новую стратегию, закрытый TEST не открывать.

## Текущее решение

ETH-кандидат прошёл walk-forward и карту устойчивости: при 0,10% положительны 90,2% сетки и все 9 ближайших к baseline точек. Однако статистика всё ещё ограничена, rolling PF составляет 1,21–1,24, концентрация лучших пяти сделок заметна, а экономический механизм не подтверждён. До решения об открытии TEST следует попытаться подтвердить или опровергнуть механизм через заранее ограниченные ETH/BTC, BTC momentum, volume и funding-признаки.
