...
Текущий исследовательский кандидат — ETHUSDT long при сильном положительном отклонении от 24-часового VWAP, медвежьем режиме BTC и высокой волатильности — **не готов к открытию TEST**.

Phase 1 устранила перекалибровку на VALIDATION. В Phase 4 экономический механизм проверен только на TRAIN+VALIDATION после явного разрешения пропустить отсутствующие в этой ветке Phase 2/3. Для 153 неперекрывающихся событий средний gross forward return равен `+0,321%` через 12 часов, но `−0,018%` через 24 часа. На 24 часах сигнал уступает контролю `BTC bear + high volatility, ETH ниже VWAP threshold` на `−0,547` процентного пункта; bootstrap-интервал широк и включает ноль.

Это не доказывает отрицательный edge, но не подтверждает специфический 24h continuation-механизм. Закрытый TEST остаётся запечатанным, paper trading преждевременен.

Актуальные подробности находятся в:
...
- **Phase 1:** аудит исполнения, причинности, данных и TRAIN-only calibration завершён; TEST не открыт.
- **Phase 2:** в текущей ветке отсутствует и не считается завершённой.
- **Phase 3:** в текущей ветке отсутствует и не считается завершённой.
- **Phase 4:** экономический механизм ETH 1h проверен на TRAIN+VALIDATION; специфическое 24h-преимущество VWAP-условия не подтверждено; TEST не открыт.

# Ближайшая последовательность работ

1. Выполнить пропущенную Phase 2: anchored/rolling walk-forward замороженного кандидата только внутри TRAIN+VALIDATION.
2. Если объединённый OOS walk-forward не положителен и не распределён по окнам, зафиксировать отказ от текущей версии без открытия TEST.
3. Только при прохождении walk-forward отдельно проверить path-dependent stop/take гипотезу без изменения текущей версии.
4. Не переходить к TEST или paper trading до отдельного явного решения.

## Текущее решение

Экономическое объяснение `ETH выше VWAP → 24h continuation` не получило поддержки: положительный conditional drift виден на 8–12 часах, но исчезает к 24 часам, а соответствующие контроли выглядят не хуже. Кандидат остаётся исследовательским и не допускается к TEST. Следующий единственный шаг — пропущенный walk-forward Phase 2; параметры и закрытый TEST не трогать.
