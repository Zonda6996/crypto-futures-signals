...
def rebuild(root: Path, workers: int) -> dict:
    ...
    manifest = {"protocol_id": PROTOCOL_ID, "partition": "development", "created_at": "frozen-restore", "files": [asdict(item) for item in records]}
    path = metadata / "raw-development-manifest.json"
    atomic_write(path, canonical(manifest))
    result = {"symbols": len(FROZEN_ROSTER), "files": len(records), "bytes": sum(row.size for row in records), "manifest_sha256": sha256_file(path)}
    if result["files"] != 3291 or result["bytes"] != 568466246:
        raise RuntimeError(f"frozen inventory mismatch: {json.dumps(result, sort_keys=True)}")
    return result
